<template>
    <i-table :opts="state.opts" ref="i-table">
        ...
        <template #i-uids="{ scope = {} }">
            <span>{{ method.count(scope?.uids) }}</span>
        </template>
...
import { getCurrentInstance, onMounted, reactive, computed, watch, defineEmits } from 'vue'
...
const state  = reactive({
    ...
    opts: {
        ...
        columns: [
            ...
            { prop: 'uids', label: '成员数', width: 200, slot: true, align: 'center' },
...
const method = {
    ...
    save: async (params = state.struct || {}) => {
        ...
        // 判断是否拥有全部的权限
        let ids = state.rules.column.map(item => parseInt(item.hash))
        // 去重去空
        ...
        select.sort((a, b) => a - b)
        // 判断是否拥有全部的权限
        if (ids.join(',') === select.join(',')) params.rules = 'all'
    ...
    edit: struct => {
        ...
        if (state.rules.column) {
            ...
            // 匹配特殊字符串隔开的
            const regex2 = /(.+)[^\w\u4e00-\u9fa5\s](.+)/
            for (const item of state.rules.column) {
    ...
    async getRules() {
        ...
        for (const item of data) {
            ...
            else if (match2) group.push(match2[1].trim())
        }
        // 去重 去空
